/**
* Due to limitations in chrome's ability to inject multiple execution scripts
* I've had to package up the tinker-chrome code with the diffDom library together
* in a single file.
* Please observe the author and license restrictions on the respective sections of code
*/



/**
* First part of script is diffDOm library
* by Fidus Writer
* https://raw.githubusercontent.com/fiduswriter/diffDOM/gh-pages/diffDOM.js
...
if(DEBUG)console.log('Registering content-script listener....');


// cache a copy of the body dom before changes are made
originalDom = null;

var eventhandler = function(request, sender, sendResponse) {
//    console.log('received message from extension: ' + JSON.stringify(request));

    switch(request.type) {
        case 'start-edit' :
            //cache dom and make everything editable...
            if(DEBUG)console.log('caching dom before edits...');
        ...
        case 'finish-edit' :
            // change to non-edit
            // create diff of body and post back to extension
            if(DEBUG)console.log('finished edits...');
            document.body.contentEditable="false";
            document.designMode="off";
            if(DEBUG)console.log('generating diff');
            var diff = dd.diff(originalDom, document.body);
            ...
            if(DEBUG)console.log('Posting diff to extension...');
        ...
        case 'apply' :
            if(DEBUG)console.log('applying dom diffs...');
                for (var i in request.diffs) {
                    var str = request.diffs[i];
                    if(DEBUG)console.log('applying diff : ' + str);
                    try {
                        dd.apply(document.body, JSON.parse(str));
                    }catch (e) {
                        console.log('unable to apply diff');
                    }
        ...
        default :
            if(DEBUG)console.log('Unknown command : ' + request.type);
...
// register our command listener to process commands from extension...
